...
> Unlimited 模式下每20刀左右cursor会收费一次，按实际使用量扣除额度。虽然没有单次请求数量限制，但仍需注意整体消耗。
