...
Rules 不是越多越好，也不是越长越好。**每条 Always Apply 的 rule 都要在每轮对话里消耗 token，而且会压缩模型处理实际代码的"工作内存"。**

核心原则：**写给「会犯的错」，而不是写给「应有的品味」。**
...
**另一个关键技巧：用"禁止/永远不要"而不是"建议/推荐"。**

模型对强指令的遵从率远高于软建议：

```
❌ 建议避免使用 class components
✅ 禁止使用 class components。如果遇到 class 组件需要修改，先把它改成函数组件再继续。
```

**用代码示例锚定，而不仅是描述。** 给模型看实际的坏例子和好例子，远比文字描述有效：
...
async function getUser(id: string) {
  const user = await db.query(`SELECT * FROM users WHERE id = ${id}`)  // SQL 注入风险
  return user
}
```
...
async function getUser(id: string): Promise<Result<User, AppError>> {
  try {
    ...
    logger.error('DB query failed', { id, error: e })
...
**如果你只记一件事**：Rules 是"每次都告诉模型"的约束，Skills/Commands 是"按需触发"的流程，两者分开是为了省 token，不是为了复杂而复杂。
